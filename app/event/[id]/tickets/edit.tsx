...
export default function EditTicketScreen() {
  ...
  const { tickets, updateTicket, getEventCapacityPools, addCapacityPool } = useTickets();
  ...
  useEffect(() => {
    if (ticket) {
      ...
      setSaleStartDate(ticket.saleStartDate);
      setSaleEndDate(ticket.saleEndDate);
      setCapacityType(ticket.capacityType);
      setSharedCapacityPoolId(ticket.sharedCapacityPoolId || '');
      setSharedCapacityLimit(ticket.sharedCapacityLimit?.toString() || '');
      setDedicatedCapacity(ticket.dedicatedCapacity?.toString() || '');
            ...
            <View style={styles.inputGroup}>
              <View style={styles.activeToggle}>
                ...
                <TouchableOpacity
                  style={[
                    styles.toggleButton,
                    isActive ? { backgroundColor: '#10b981' } : { backgroundColor: '#ef4444' },
                  ]}
                  onPress={() => setIsActive(!isActive)}
                >
                  <Text style={styles.toggleText}>{isActive ? 'Activo' : 'Inactivo'}</Text>
                </TouchableOpacity>
              </View>
            </View>
